...
public final class BluetoothShare implements BaseColumns
{
	...
	//Broadcast Action: this is sent by the Bluetooth Share component to transfer complete. The request detail could be retrieved by app
	//as _ID is specified in the intent's data.
	public static final String TRANSFER_COMPLETED_ACTION = "android.btopp.intent.action.TRANSFER_COMPLETE";
	
	//This is sent by the Bluetooth Share component to indicate there is an incoming file need user to confirm.
	...
	//This is sent by the Bluetooth Share component to indicate there is an incoming file request timeout and need update UI.
	...
	//The name of the column containing the filename that the incoming file request recommends. When possible, the Bluetooth Share manager will
	//attempt to use this filename, or a variation, as the actual name for the file.
	...
	//The name of the column containing the flags that controls whether the transfer is displayed by the UI. See the VISIBILITY_* constants for a
	// list of legal values.
	public static final String VISIBILITY = "visibility";
	
	//The name of the column containing the current user confirmation state of the transfer. Applications can write to this to confirm the transfer. the
	//USER_CONFIRMATION_* constants for a list of legal values.
	public static final String USER_CONFIRMATION = "confirm";
	
	//The name of the column containing the current status of the transfer. Applications can read this to follow the progress of each download. See
	...
	//This transfer is visible and shows in the notifications while in progress and after completion.
	...
	//Returns whether the transfer is suspended. (i.e. whether the transfer won't complete without some action from outside the transfer manager).
	...
	//This transfer has successfully completed. Warning: there might be other status values that indicate success in the future. Use isSucccess() to
	...
	//This transfer cannot be performed because the length cannot be determined accurately. This is the code for the HTTP error "Length Required", which
	//is typically used when making requests that require a content length but don't have one, and it is also used in the client when a response is
	//received whose length cannot be determined accurately (therefore making it impossible to know when a transfer completes).
	public static final int STATUS_LENGTH_REQUIRED = 411;
	
	//This transfer was interrupted and cannot be resumed. This is the code for the OBEX error "Precondition Failed", and it is also used in situations
	...
	//This transfer has completed with an error. Warning: there will be other status values that indicate errors in the future. Use isStatusError() to
	//capture the entire category.
	public static final int STATUS_UNKNOWN_ERROR = 491;
	
	//This transfer couldn't be completed because of a storage issue. Typically, that's because the file system is missing or full.
	...
	//This transfer couldn't be completed because of an error receiving or processing data at the OBEX level.
